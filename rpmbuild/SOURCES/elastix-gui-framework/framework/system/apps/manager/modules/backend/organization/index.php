...
//            $num_queues=$num_queues+0;
        ...
        $exito=$pOrganization->createOrganization($name,$domain,$country,$state,$address,$country_code,$area_code,$num_user,$admin_password);
        if($exito!==false){
...
//        $email_contact = trim(getParameter("email_contact"));
        ...
        if($country=="0" || !isset($country)){
            ...
            $exito=$pOrganization->setOrganization($idOrg,$name,$country,$city,$address,$country_code,$area_code,$quota,$num_user,$num_exten,$num_queues,$credentials['userlevel']);
            if($exito)
            {
                $smarty->assign("mb_title", _tr("Message"));
