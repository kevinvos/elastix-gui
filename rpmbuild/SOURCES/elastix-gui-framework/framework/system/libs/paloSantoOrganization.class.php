...
class paloSantoOrganization{
    ...
    */
	...
	print $sComando;
        ...
        if ($ret != 0){
            print"<pre>";print_r(implode('',$output));print"</pre>";die;
    ...
    function createOrganization($name,$domain,$country,$city,$address,$country_code,$area_code,$max_num_user,$admin_password)
    {
        global $arrConf;
        ...
        if(array($resOrgz) && count($resOrgz)==0){
            ...
            if (!$this->_DB->genQuery(
                'INSERT INTO organization (name,domain,code,idcode,country,city,address,state) '.
                'VALUES (?,?,?,?,?,?,?,?)',
                array($name, $domain, $pbxcode, $idcode, $country, $city, $address, 'active'))) {
                $this->_DB->rollBack();
                $this->errMsg = $this->_DB->errMsg;
            }else{
...
//                            $error=$this->errMsg;
//                            print"<pre>";print_r($this->errMsg);print"</pre>";die;
...
//                        }else{
                            ...
                            $user=$this->createAdminUserOrg($resultOrgz['id'],$domain,$name,$admin_password,$country_code,$area_code,true);
                            if($user){
                                $this->_DB->commit();
                                return true;
                            }else{
...
//                        }
    ...
    private function createAdminUserOrg($idOrg,$domain,$CompanyName,$password,$country_code,$area_code){
...
//            }
    ...
    function setOrganization($id,$name,$country,$city,$address,$country_code,$area_code,$quota,$max_num_user,$max_num_exten,$max_num_queues,$userLevel1)
    {
